...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-xl font-semibold text-white">Laporan Rekap</h1>
    ...
    <div class="flex gap-2">
        <a href="{{ route('admin.laporan.pdf', request()->query()) }}"
           class="inline-flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20
                  text-red-400 text-sm rounded-lg transition-colors border border-red-500/20">
            ...
            <a href="{{ route('admin.laporan.excel', request()->query()) }}"
               class="inline-flex items-center gap-2 px-4 py-2 bg-green-500/10 hover:bg-green-500/20
              text-green-400 text-sm rounded-lg transition-colors border border-green-500/20">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<form method="GET" class="flex flex-wrap gap-3 mb-6">
    <select name="bulan"
            class="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm
                   text-gray-200 focus:outline-none focus:border-teal-500">
    ...
    <button type="submit"
            class="px-4 py-2 bg-teal-500 hover:bg-teal-400 text-gray-950 text-sm
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    @foreach([
        ['label'=>'Total Pengukuran', 'value'=>$rekap['total'],      'color'=>'text-white'],
        ['label'=>'Normal',           'value'=>$rekap['normal'],      'color'=>'text-teal-400'],
        ['label'=>'Stunting',         'value'=>$rekap['stunting'],    'color'=>'text-orange-400'],
        ['label'=>'Gizi Buruk/Kurang','value'=>$rekap['gizi_buruk'], 'color'=>'text-red-400'],
    ] as $c)
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="border-b border-gray-800">
                <th class="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Balita</th>
                <th class="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Posyandu</th>
                ...
                <th class="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Tanggal</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-800">
            ...
            <tr class="hover:bg-gray-800/40 transition-colors">
                <td class="px-5 py-3.5">
                    <p class="text-gray-200 font-medium">{{ $p->balita->nama }}</p>
                    <p class="text-xs text-gray-500">{{ $p->balita->usiaBulanPada($p->tanggal_ukur) }} bulan</p>
                </td>
                ...
                <td class="px-5 py-3.5 text-center text-gray-400 hidden lg:table-cell">
                    {{ $p->zscore_bbu ?? '—' }}
                </td>
                <td class="px-5 py-3.5 text-center text-gray-400 hidden lg:table-cell">
                    {{ $p->zscore_tbu ?? '—' }}
